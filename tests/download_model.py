...
def model_download(model_id, local_dir="/data", hub_name="hf", repo_type="model"):
    ...
    if hub_name == "hf":
        ...
        cmd = f"huggingface-cli download --repo-type {repo_type} --resume-download {model_id} --local-dir {local_dir}/{model_id} --local-dir-use-symlinks False --token {token}"
        # 启动下载
        os.system(cmd)
        print("下载完成！")
...
if __name__ == "__main__":
    ...
    data_model_id_list = ["RedHatAI/Qwen3.6-35B-A3B-NVFP4"]

    for model_id in data_model_id_list:
        # 设置仓库id
        model_download(model_id, local_dir, hub_name="modelscope", repo_type=repo_type)
    print("所有下载完毕！")
